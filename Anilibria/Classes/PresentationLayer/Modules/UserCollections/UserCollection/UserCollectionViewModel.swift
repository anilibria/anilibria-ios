...
final class UserCollectionPart: DIPart {
    static func load(container: DIContainer) {
        container.register(UserCollectionViewModel.init)
...
final class UserCollectionViewModel: UserCollectionViewModelProtocol {
    ...
    private let service: UserCollectionsService
    ...
    init(service: UserCollectionsService) {
        ...
        delete = { [weak self] item in
            self?.removeFromList(series: item)
        }

        service.collectionsUpdates().sink { [weak self] updates in
            guard let self else { return }
            switch updates {
            case .added(let series, let toType, _) where toType == collectionType:
                append(series: series)
            case .added(let series, _, let fromType) where fromType == collectionType:
                remove(series: series)
            case .deleted(let series, let type) where type == collectionType:
                remove(series: series)
            default:
                break
            }
    ...
    func bind(type: UserCollectionType, router: UserCollectionRoutable) {
    ...
    private func load(with activity: ActivityDisposable?) {
        ...
        nextPage = 1
        pagination.reset()
        pageSubscriber = service.fetchSeries(
            type: collectionType,
            limit: limit,
            page: nextPage,
            data: searchData
    ...
    private func removeFromList(series: Series) {
        guard let collectionType else { return }
        service.removeFrom(collection: collectionType, series: series)
            .manageActivity(activityBehavior?.showLoading(fullscreen: false))
...
extension UserCollectionViewModel: RouterCommandResponder {
